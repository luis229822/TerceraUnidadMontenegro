...
<div class="max-w-7xl mx-auto py-6 px-0">
    <h1 class="text-2xl font-bold text-center text-gray-800 mb-6">Consulta de Oficinas</h1>
    ...
    <div id="oficinas-table" class="overflow-x-auto">
        @include('profile.partials.oficinas-table')
    </div>
</div>
...
$(document).ready(function () {
    function fetchData(search = '', page = 1) {
        $.ajax({
            url: "{{ route('oficinas.index') }}",
            type: "GET",
            data: { search: search, page: page },
            success: function (response) {
                $("#oficinas-table").html(response.html);
